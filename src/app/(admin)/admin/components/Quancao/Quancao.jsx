...
function Quancao() {
    ...
    const handgetQuancao = async () => {
        setLoading(true)
        try {
            const data = await ApiQuancao.getThuonghieumenu()
            setdataquancao(data)
            console.log(data)
            setLoading(false)
    ...
    useEffect(() => {
        ...
        const filtered = dataquancao.filter((quancao) =>
            quancao.label.toLowerCase().includes(searchTerm.toLowerCase())
        )
    ...
    const handThemthuonghieumenu = async () => {
        setLoading(true)
        try {
            if (!image && !tinhtrangs && !lable && !iddienthoai && !text) {
                ...
                if (dataimage) {
                    ...
                    const datathuonghieu = await ApiQuancao.postThuonghieumenu(images, tinhtrangs, lable, iddienthoai, text)
    ...
    const getIdquancaothuonghieu = async (id) => {
        setLoading(true)
        try {
            const datathuonghieu = await ApiQuancao.getIdThuonghieumenu(id)
            setiddienthoai(datathuonghieu.dienthoaiId)
            setlable(datathuonghieu.label)
            settinhtrang(datathuonghieu.tinhtrang)
            setimage(datathuonghieu.hinhanh)
            settext(datathuonghieu.text)
            setdatacapnhatquancao(datathuonghieu)
            setLoading(false)
        } catch {
    ...
    const putchinhsuathuonghieub = async (id) => {
        ...
        try {
            if (image === undefined || image === datacapnhatquancao.hinhanh) {
                ...
                const responsecanhat = await ApiQuancao.putThuonghieumenu(id, tinhtrangs, lable, iddienthoai, text, capnhathinhanh);
                ...
                if (responsedelete) {
                    ...
                    if (responsehinhanh) {
                        ...
                        const responsecanhat = await ApiQuancao.putThuonghieumenu(id, tinhtrangs, lable, iddienthoai, text, capnhathinhanh);
                        setMessage('Cập nhật thương hiệu menu thành công')
                        handgetQuancao()
                        setTimeout(() => {
                            setMessage('')
                        }, 5000);
            ...
            setMessage('Cập nhật thương hiệu menu không thành công')
            setLoading(false)
            setTimeout(() => {
                setMessage('')
            }, 5000);
    ...
    const handdelete = async (id, hinhanh) => {
        ...
        try {
            ...
            if (responsedelete) {
                const deletethuonghieu = await ApiQuancao.deleteThuonghieumenu(id)
    ...
    return (
        <div>
            ...
            <div>
                <h2 className='text-4xl mt-3 mb-3'>Quản cáo thương hiệu menu điện thoại</h2>
            </div>
            {themquancao && (
                <div className="loading-themquancao">
                    <div className='!z-5 relative loading-themquancaos flex h-full w-full bg-slate-950 flex-col rounded-[20px] bg-clip-border p-4 shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none'>
                        ...
                        <div className='mb-auto flex-col items-center justify-center'>
                            <h2 className='textthemquancao'>Thêm thương hiệu menu điện thoại</h2>
                            <div className='mt-9'>
                                <label className="text-xl text-white dark:text-gray-200" htmlFor="username">Tên thương hiệu</label>
                                <input id="username" type="text" onChange={(e) => setlable(e.target.value)} placeholder='Tên thương hiệu' className="block w-full px-4 py-2 mt-1 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" />
                            </div>
                            <div className='mt-9'>
                            ...
                            <div className='mt-9'>
                                <label className="text-xl text-white dark:text-gray-200" htmlFor="username">Nôi dung thương hiệu đi kèm</label>
                                <input id="username" type="text" onChange={(e) => settext(e.target.value)} placeholder='Tên thương hiệu' className="block w-full px-4 py-2 mt-1 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" />
                            </div>
                            <div className='mt-9'>
                            ...
                            <div className='mt-9'>
                                <button onClick={handThemthuonghieumenu} className="button-themthuonghieu px-8 py-4 leading-5 text-white transition-colors duration-200 transform bg-green-500 rounded-md hover:bg-green-800 focus:outline-none focus:bg-gray-600">Thêm</button>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div>
                <h2 className='text-xl mt-3 mb-3'>Thêm quản cáo thương hiệu menu điện thoại</h2>
                <div>
                    <button onClick={handHienfrom} className="button-themquancao text-2xl px-3 py-3 leading-5 text-white transition-colors duration-200 transform bg-orange-700 rounded-md hover:bg-yellow-300 focus:outline-none focus:bg-yellow-300">Thêm quản cáo menu điện thoại</button>
                </div>
            </div>
            <div className="mb-4">
                <input
                    type="text"
                    placeholder="Tìm kiếm theo tên thương hiệu (lable)"
                    value={searchTerm}
                    ...
                    <tbody>
                        {currentItems.map((quancao) => (
                            <tr key={quancao.id}>
                                ...
                                <td className="border text-black px-2 align-middle border-gray-200 text-base whitespace-nowrap text-left text-blueGray-700">
                                    {quancao.label}
                                </td>
                                <td className="border text-black px-2 align-middle border-gray-200 text-base whitespace-nowrap text-left text-blueGray-700">
                                    {quancao.text}
                                ...
                                <td className="border text-black px-1 align-middle border-gray-200 text-xs whitespace-nowrap p-4 text-left text-blueGray-700">
                                    {suaquancao[quancao.id] && (
                                        <div className="loading-themquancao">
                                            <div className='!z-5 relative loading-themquancaos flex w-full flex-col rounded-[20px] bg-clip-border p-4 shadow-3xl shadow-shadow-500 dark:!bg-navy-800 dark:text-white dark:shadow-none'>
                                                <div className='ml-auto'>
                                                    <div className='relative flex'>
                                                        <div className="mb-3 text-right">
                                                ...
                                                <div className='mb-auto flex-col items-center justify-center'>
                                                    ...
                                                    <div className='mt-9'>
                                                        <label className="text-xl text-black dark:text-gray-200" htmlFor="username">Tên thương hiệu</label>
                                                        <input id="username" value={lable} type="text" onChange={(e) => setlable(e.target.value)} placeholder='Tên thương hiệu' className="block w-full px-4 py-2 mt-1 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" />
                                                    </div>

                                                    <div className='mt-9'>
                                                        <label className="text-black text-xl dark:text-gray-200" htmlFor="username">Chọn hình ảnh</label>
                                                        <div className='mt-2'>
                                                    ...
                                                    <div className='w-full mt-9'>
                                                        <label className="text-black text-xl dark:text-gray-200" htmlFor="passwordConfirmation">Chọn sản phẩm</label>
                                                        <select value={iddienthoai} onChange={handChaniddienthoai} className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                                                            <option hidden>Chọn sản phẩm của thương hiệu</option>
                                                            {datadienthoai.map((dienthoai) => {
                                                                return (
                                                    ...
                                                    <div className='mt-9'>
                                                        <label className="text-xl text-black dark:text-gray-200" htmlFor="username">Nôi dung thương hiệu đi kèm</label>
                                                        <input id="username" type="text" value={text} onChange={(e) => settext(e.target.value)} placeholder='Nôi dung thương hiệu đi kèm' className="block w-full px-4 py-2 mt-1 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring" />
                                                    </div>
                                                    <div className='mt-9'>
                                                        <label className="text-black text-xl dark:text-gray-200" htmlFor="passwordConfirmation">Trạng khái hiện thị điện thoại</label>
                                                        <select value={tinhtrangs || ''} onChange={onChantinhtrang} className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                                                            <option hidden>Chọn tình trạng hiện thị máy</option>
                                                            <option value="Mở">Mờ</option>
                                                            <option value="Ẩn">Ẩn</option>
